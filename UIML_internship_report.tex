...
% see https://tex.stackexchange.com/questions/112576/math-mode-in-tabular-without-having-to-use-everywhere
\newcolumntype{C}{>{$}c<{$}}
\newcolumntype{L}{>{$}l<{$}}
...
Calculator \citep{github:uiml}. The objective of my internship was to simplify the formulas computed by the calculator using Coq, ensuring that the simplification is equivalent to
the original formula, and thus remains a uniform interpolant.
...
$\entails$ being logical entailment.

While for this internship we only work with IPC logic this theorem remains valid pour other logics such us iSL, K and GL.
...
| (_, Implies Bot _) => Lt
| (And f1 f2, f3) =>
    match (obviously_smaller f1 f3, obviously_smaller f2 f3) with
    | (Lt, _) | (_, Lt) => Lt
    | (Gt, Gt) => Gt
    | _ => Eq
    end
